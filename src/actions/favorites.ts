...
// FIX: la query anterior tenía 4 placeholders pero solo 3 args → error en Turso
export async function getFavoritePosts(userId: number): Promise<Post[]> {
  ...
  try {
    const result = await db.execute({
      sql: `
        ...
        SELECT
          ...
          MAX(CASE WHEN l.user_id = ?1 THEN 1 ELSE 0 END) AS has_liked,
          MAX(CASE WHEN f.follower_id = ?2 THEN 1 ELSE 0 END) AS is_following_author,
          1                                            AS has_favorited,
        ...
        LEFT JOIN follows f      ON p.user_id = f.following_id AND f.follower_id = ?3
        WHERE fav.user_id = ?4
        GROUP BY p.id, fav.created_at
        ORDER BY fav.created_at DESC
      `,
      // 4 parámetros posicionales con la sintaxis ?N de libSQL
      args: [currentUserId, currentUserId, currentUserId, userId],
    });
    ...
    console.error('[getFavoritePosts] Error al consultar favoritos:', err);
    // Retornar array vacío para no colapsar la página
    return [];
  }
}
