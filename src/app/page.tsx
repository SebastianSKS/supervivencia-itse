...
export default async function HomePage({ searchParams }: Props) {
  ...
  let veterans: Awaited<ReturnType<typeof getTopVeterans>> = [];
  let loadError: string | null = null;

  try {
    [session, posts, veterans] = await Promise.all([
      getSession(),
      getPosts(sort),
      getTopVeterans(),
    ]);
  ...
  return (
    <div className="min-h-screen bg-zinc-950">
      ...
      <div className="max-w-6xl mx-auto px-4 pt-2 pb-12 sm:pb-16">
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-start">

          {/* ── Columna Principal: El Muro ──────────────────────────────────── */}
          <section id="muro" className="flex-1 min-w-0">
      ...
      <footer className="border-t border-white/[0.04]">
        <div className="max-w-6xl mx-auto px-4 py-6 flex flex-col sm:flex-row items-center justify-between gap-2 text-center sm:text-left">
